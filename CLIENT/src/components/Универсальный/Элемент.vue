<template>
    <div style="padding: 10px;">
        ...
        <BaseDialog :props="props" v-model:МодельОбъекта="МодельОбъекта"
        ...
        <div style="display: grid; grid-template-columns: 1fr 5fr; gap: 5px; padding: 10px; margin: 5px;">
            <template v-for="curFiled of props.ОкноСОбъектом.Объект.ПредставлениеЭлемента.НастройкаПолей"
                ...
                <EnterField :styleForInput="curFiled.Стили ? curFiled.Стили : ''"
                    :attrForInput="curFiled.Атрибуты ? curFiled.Атрибуты : ''" v-model:id=МодельОбъекта[curFiled.Имя]
                    :ТаблицаВнешнегоКлюча=curFiled.ТаблицаВнешнегоКлюча v-model:МодельОбъекта=МодельОбъекта
...
function ДобавитьСтрокуВПодчиненнуюТаблицу(НастройкаПодчиненнойТаблицы, ДанныеПодчиненнойТаблицы) {
    const ДанныеСтроки = {};
    ДанныеСтроки[НастройкаПодчиненнойТаблицы.КолонкаСВнешнимКлючемВПодчиненнойТаблице] = МодельОбъекта.value[НастройкаПодчиненнойТаблицы.КлючГлавнойТаблицы];
...
const ВычислитьЗаголовок = () => {
    if (typeof props.ОкноСОбъектом.Объект.ПредставлениеЭлемента.Заголовок === 'function')
        Заголовок.value = props.ОкноСОбъектом.Объект.ПредставлениеЭлемента.Заголовок(МодельОбъекта.value);
    else
