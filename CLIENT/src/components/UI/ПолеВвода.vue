...
function ОбработчикНажатияПоКнопкеОткрытия() {
    const ДиалоговоеОкно: ДиалоговоеОкно = {
        ...
        ТипКомпонентаПредставления: ТипКомпонентаПредставления.Список
    };
    ...
    СоздатьДиалоговоеОкно(
        {
            ...
            ОбработчикВыбораВнешнегоКлюча: (selRecord) => {
                ...
                props.ОбработчикПослеЗаполненияВнешнегоКлюча ? props.ОбработчикПослеЗаполненияВнешнегоКлюча(selRecord, МодельОбъекта.value) : () => { }
            },
